...
clogd.mgarch1 <- cmpfun(logd.mgarch1)
...
clogd.mgarch2 <- cmpfun(logd.mgarch2)
...
clogd.mgarch3 <- cmpfun(logd.mgarch3)
...
clogd.mgarch4 <- cmpfun(logd.mgarch4)


## Just replacing my stupid rev
...
clogd.mgarch5 <- cmpfun(logd.mgarch5)


## try to replace all the loops with filter
logd.mgarch6 <- function(xt, beta, pt, which){
    xt <- as.numeric(xt)
    T <- length(xt)
    lpt <- length(pt)
    lb <- length(beta)
...
#                              xt[1:(i - 1)]^2)
        ...
        cp.beta2 <- c(0, as.numeric(filter(xt[1:(T - 1)]^2,
                                           beta[2], "recursive")))
        cp.beta[1:2] <- 0
        dsigmadalpha0 <- cumsum(c(0, beta[2]^(0:(T-2)))) #1
        dsigmadalpha1 <-
...
clogd.mgarch6 <- cmpfun(logd.mgarch6)


## try to replace all the rep with rep.int
logd.mgarch7 <- function(xt, beta, pt, which){
    xt <- as.numeric(xt)
    T <- length(xt)
    lpt <- length(pt)
    lb <- length(beta)
...
clogd.mgarch7 <- cmpfun(logd.mgarch7)



## Test the difference when cp.beta is removed, it appears very
## significant!
logd.mgarch8 <- function(xt, beta, pt, which){
    xt <- as.numeric(xt)
    T <- length(xt)
    lpt <- length(pt)
    lb <- length(beta)
...
## Use Convolution for the other loop, this is now the new benchmark
## and the function being implemented
logd.mgarch9 <- function(xt, beta, pt, which){
    xt <- as.numeric(xt)
    T <- length(xt)
    lpt <- length(pt)
    lb <- length(beta)
...
clogd.mgarch9 <- cmpfun(logd.mgarch9)
...
check <- rnorm(1000)
benchmark(logd.mgarch1(check, rep(0.5, 5), 1, which = rep(1, 4)),
          ...
          logd.mgarch5(check, rep(0.5, 5), 1, which = rep(1, 4)),
          logd.mgarch6(check, rep(0.5, 5), 1, which = rep(1, 4)),
          logd.mgarch7(check, rep(0.5, 5), 1, which = rep(1, 4)),
          logd.mgarch8(check, rep(0.5, 5), 1, which = rep(1, 4)),
          logd.mgarch9(check, rep(0.5, 5), 1, which = rep(1, 4)),
          replications = 10, order = "relative")


## Now test the improvement in cnmms
library(fGarch)


## obs = 500
tdg <- as.numeric(data.matrix(dem2gbp[1:500,]))
tmdg <- tdg
class(tmdg) <- "mgarch"

## 100
logd.mgarch <- logd.mgarch1
system.time(
cnmms(tmdg, plot = "gradient", grid = 10, verb = 4)
)
## 25
logd.mgarch <- cmpfun(logd.mgarch9)
system.time(
cnmms(tmdg, plot = "gradient", grid = 10, verb = 4)
)

## obs = 1000
tdg <- as.numeric(data.matrix(dem2gbp[1:1000]))
tmdg <- tdg
class(tmdg) <- "mgarch"
...
## This shows that the size of the data doesn't really matter! why?
## Most of the time spent on optimisation? the likelihood too flat?



## Check the compiler library
...
loopRec2 <- function(x, alpha){
    ...
    y <- numeric(n)
    for(i in seq_len(n)){
        y[i] <- sum(cumprod(rep(alpha, i)) * x[i:1])
    }
    y
}
...
## give convolve a test
xt = rnorm(1000, 0, 0.2)
alpha = 0.8
myloop <- function(xt, alpha){
    T <- length(xt)
    rlt <- double(T)
    for(i in 1:T){
        rlt[i] = sum(1:i * alpha^(0:(i - 1)) * xt[i:1]^2)
    }
    rlt
}


myconvolve <- function(xt, alpha){
    T <- length(xt)
    myfilter = 1:T * alpha^(0:(T - 1))
    rlt <- convolve(myfilter, rev(xt^2), type = "open")[1:T]
    rlt
}

a <- myloop(xt, alpha)
b <- myconvolve(xt, alpha)


plot(a, type = "l")
lines(b, col = "red", lty = 2)

benchmark(myloop(xt, alpha), myconvolve(xt, alpha),
          replications = 100, order = "relative")
