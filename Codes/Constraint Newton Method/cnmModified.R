...
cnmms <- function(x=rcvp2(), init=NULL, maxit=1000,
    ...
    if(length(mix$pt) < kmax) {
      gridpoints = seq(0.3, 20, length = grid)
      g = maxgrad(x, beta, dmix, ma, grid=gridpoints, tol=-Inf)
      # g = maxgrad2(x, beta, dmix, ma, mix$pt, tol=-Inf)
...
#        print(r$beta)
#        print(sc)
#        if(!is.null(r$beta) && valid(x, r$beta * sc, r$mix)){
#            r$mix$pt <- r$mix$pt/sc
#            r$beta[c(1, 3, 4)] <- r$beta[c(1, 3, 4)] * sc
#        }
#        else if(!is.null(r$beta)){
#            r$mix$pt <- r$mix$pt/sc
#            r$beta[4] <- r$beta[4] * sc
#        }
#        print(r$mix$pt)
#        print(r$beta)
