...
logd.mgarch3 <- function(xt, beta, pt, which){
    ...
    names(dl) <- c("ld", "db1", "dt1", "dt2", "sigma.t", "dbvec", "sig.vec")
...
#             c(0, 0, 1:(T - 2) * c(1, cumprod(rep(beta[2], T - 3))) * beta[4]) +
                 c(0, 1:(T - 1) * beta[2]^(0:(T - 2)) * beta[4]) + cp.beta #2
        cp.beta2 <- double(T)
        cp.beta2[1] <- 0
        for(i in 2:T){
        ...
        #     2 * sqrt(beta[4]) #4
        dsigmadsigma <- c(2 * sqrt(beta[4]), beta[2]^(1:(T - 1)))
...
logd.mgarch2 <- function(xt, beta, pt, which){
    ...
    sigma.t <- double(T) ## The conditional standard deviation
    # sigma.t[1] <- beta[4] ## Let it be beta[4] for now
    sigma.t[1] <- beta[4]^2 ## Let it be beta[4] for now  # yong
    for(i in 2:T){
        sigma.t[i] <- beta[1] + beta[2] * sigma.t[i - 1] +
            beta[3] * xt[i - 1]^2
    }
...
#             c(0, 0, 1:(T - 2) * c(1, cumprod(rep(beta[2], T - 3))) * beta[4]) +
#                  c(0, 1:(T - 1) * beta[2]^(0:(T - 2)) * beta[4]) + cp.beta #2
                 c(0, 1:(T - 1) * beta[2]^(0:(T - 2)) * beta[4]^2) + cp.beta #2 yong
        cp.beta2 <- double(T)
        cp.beta2[1] <- 0
        for(i in 2:T){
            cp.beta2[i] <- sum(beta[2]^(0:(i - 2)) *
        ...
        #     2 * sqrt(beta[4]) #4
        # dsigmadsigma <- c(2 * sqrt(beta[4]), beta[2]^(1:(T - 1)))
        dsigmadsigma <- 2 * beta[4] * beta[2]^(0:(T - 1)) # yong
        ...
        #                dsigmadsigma), "*")
        # dldsigma <- array(dldsigma/sig.vec, dim = c(T, lpt, lb))
        dldsigma <- array(dldsigma / sig.vec, dim = c(T, lpt, lb)) # yong
        #dl$dldsigma <- dldsigma
        dl$db1 <- dldsigma * dbvec
        dl$dbvec <- dbvec
        dl$sig.vec <- sig.vec
...
logd.mgarch <- function(xt, beta, pt, which){
  ...
  lb <- length(beta)
  dl <- vector("list", length = 7)
  names(dl) <- c("ld", "db1", "dt1", "dt2", "sigma", "dbvec", "sig.vec")
  pt2 = pt^2
  xt2 = xt^2
  sigma2 <- double(T) 
  sigma2[1] <- beta[4]^2
  beta13.xt2 = beta[1] + beta[3] * xt2
  for(i in 2:T) sigma2[i] <- beta[2] * sigma2[i-1] + beta13.xt2[i-1]
  sigma <- sqrt(sigma2)
  dl$sigma <- sigma
  if(which[1] == 1) {
    dl$ld <- -0.5 * log(2 * pi * sigma2) - rep(log(pt), each = T) -
      xt2 / sigma2 / rep(2 * pt2, each = T)
    dim(dl$ld) = c(T, lpt)
  }
  if(which[2] == 1){
    dldsigma <- xt2 / sigma^3 / rep(pt2, each = T) - 1 / sigma
    dim(dldsigma) = c(T, lpt)
    dl$dldsigma <- dldsigma
    sig.vec <- 2 * sigma
    # cp.alpha = cumsum(c(0, beta[2]^(0:(T-2))))
    cp.alpha <- c(0,(1-beta[2]^(1:(T-1))) / (1-beta[2]))
    dsigmadalpha0 <- cp.alpha
    cp.beta <- double(T)
    cp.beta[1:2] <- 0
    for(i in 3:T)
      cp.beta[i] <- beta[3] * sum((1:(i - 2)) * beta[2]^(0:(i - 3)) *
    ...
    dsigmadalpha1 <-
      (beta[1] * cumsum(c(0, 0, 1:(T - 2) * beta[2]^(0:(T - 3))))) +
        c(0, 1:(T - 1) * beta[2]^(0:(T - 2)) * sigma2[1]) + cp.beta
    cp.beta2 <- double(T)
    cp.beta2[1] <- 0
    for(i in 2:T)
      cp.beta2[i] <- sum(beta[2]^(0:(i - 2)) * rev(xt2[1:(i - 1)]))
    dsigmadbeta1 <- cp.beta2 
  ...
  if(which[3] == 1){
    dt1 = xt2 / sigma2 / rep(pt^3, each=T) - 1 / rep(pt, each=T)
    dim(dt1) = c(T,lpt)
    dl$dt1 = dt1
  }
  if(which[4] == 1){
    dt2 <- -3 * xt2 / sigma2 / rep(pt^4, each = T) + 1 / (rep(pt2, each = T))
    dim(dt2) = c(T, lpt)
    dl$dt2 = dt2
  }
  dl
...
## Testing version to see whether it's the constraints that are
## causing the pre-mature break in the BFGS
#valid.mgarch <- function(x, beta, mix) TRUE
...
# range.mgarch = function(x, beta) c(0.1,10)
